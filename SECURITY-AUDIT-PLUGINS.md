...
| 11 | docker-plugin | mcp-server-docker (uvx) | **CRITICAL** |
...
- Mount host volumes into containers
- Access Docker socket (/var/run/docker.sock)

**Risk:** Container escape, host-level access, data exfiltration

**Recommendation:** Restrict to sandboxed environments only. Do NOT use in production with sensitive data.
...
1. **Fix broken packages** - Replace `@anthropic-ai/*` references with verified alternatives or remove until official packages are available

2. **Add Docker warning** - Document that docker-plugin should only be used in isolated environments
...
|------------|-------|---------|
| **CRITICAL** | 1 | docker-plugin |
| **BROKEN** | 4 | git-workflow, memory, remotion, video-generator |
...
| NONE | 23 | All security/knowledge plugins |
