...
if [ -d "$STARTER_PACK_DIR/skills" ] && [ -d "$STARTER_PACK_DIR/plugins" ]; then
    ...
    if [ -f "$STARTER_PACK_DIR/scripts/update-mac.sh" ]; then
        mkdir -p "$HOME/.local/bin"
        cp "$STARTER_PACK_DIR/scripts/update-mac.sh" "$HOME/.local/bin/update-rising-tides"
    ...
    if [ "$DOWNLOAD_SUCCESS" = true ]; then
        ...
        # Install update-rising-tides command
        if [ -f "$TEMP_DIR/scripts/update-mac.sh" ]; then
            mkdir -p "$HOME/.local/bin"
            cp "$TEMP_DIR/scripts/update-mac.sh" "$HOME/.local/bin/update-rising-tides"
            chmod +x "$HOME/.local/bin/update-rising-tides"
            print_success "Installed 'update-rising-tides' command"
        fi
